...
import { AllOfficials } from '@/actions/official-action';

interface OfficialAutoSelectProps {
  users: AllOfficials[];
  onSelect: (user: AllOfficials) => void;
  placeholder?: string;
}

function OfficialAutoSelect({
  users,
  onSelect,
  placeholder = 'Select a user...'
}: OfficialAutoSelectProps) {
  const [open, setOpen] = React.useState(false);
  const [selectedUser, setSelectedUser] = React.useState<AllOfficials | null>(
    null
  );
  const triggerRef = React.useRef<HTMLButtonElement>(null); // Ref untuk button trigger

  const handleSelect = (user: AllOfficials) => {
            ...
            <CommandGroup>
              {users.map((user) => (
                ...
                >
                  ...
                  />
                  [{user.region.name}] - {user.fullName}
                </CommandItem>
              ))}
            </CommandGroup>
          </CommandList>
...
export default OfficialAutoSelect;
