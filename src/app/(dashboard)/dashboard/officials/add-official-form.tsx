...
export function AddOfficialForm({ onSuccess, regions }: AddOfficialFormProps) {
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const router = useRouter();
  const { session } = useCurrentSession();
  const form = useForm<OfficialFormDataCreate>({
    resolver: zodResolver(officialSchemaCreate),
    defaultValues: {
      fullName: '',
      address: '',
      phone: '',
  ...
  useEffect(() => {
    if (userRegions.length > 0) {
      ...
      form.setValue('regionId', userRegions[0]);
    }
  }, [userRegions, form.setValue]);
  ...
  const onSubmit = async (values: OfficialFormDataCreate) => {
    const formData = createFormDataOfficialInput(values);
    await axios.post('/api/official', formData, {
      headers: {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-8'>
        ...
        />
        <Button disabled={form.watch('aggree') !== true} type='submit'>
          Submit
        </Button>
      </form>
    </Form>
